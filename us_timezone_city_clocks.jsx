...
function safeFormatTime(date: Date, timeZone: string) {
  try {
...
function safeFormatDate(date: Date, timeZone: string) {
  try {
    return new Intl.DateTimeFormat("en-US", {
      timeZone,
      weekday: "long",
...
export default function USTimezoneStateClocks() {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {STATE_TIMEZONES.map((item) => (
            <Card
              key={item.zoneLabel}
              className="rounded-3xl border border-white/10 bg-white/5 text-white"
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm text-zinc-400">{item.zoneLabel}</p>
                    <CardTitle className="mt-1 text-xl font-semibold sm:text-2xl">
